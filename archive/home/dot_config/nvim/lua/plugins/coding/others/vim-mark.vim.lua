return {
    ...
    keys = function()
        ...
        map.group('<leader>m',                                            { group = "[m]ARKS" })
        map.map('n',         '<Leader>mM', ':Mark<space>',                { desc  = "?" })
        -- No default mapping for Marks
        map.map('n',         '<Leader>mp', map.cmd('Marks'),              { desc  = "[p]review marks" })
        map.map('n',         '<Leader>mn', ':MarkName<space>',            { desc  = "[n]ame mark" })
        map.map('n',         '<Leader>ms', ':MarkSave<space>',            { desc  = "[s]ave marks" })
        map.map('n',         '<Leader>ml', ':MarkLoad<space>',            { desc  = "[l]oad marks" })
        ...
        map.map({'n', 'x'},  '<Leader>mm', '<plug>MarkSet',               { desc  = "[m]ark set/remove" })
        -- No defult mapping for <Plug>MarkIWhiteSet.
        map.map({'n', 'x'},  '<Leader>mr', '<plug>MarkRegex',             { desc  = "[r]egex mark" })
        map.map('n',         '<Leader>mc', '<plug>MarkConfirmAllClear',   { desc  = "[c]lear all marks (requires confirmation)" })
        -- No default mapping for '<Plug>MarkAllClear / MarkClear.
        map.map('n',         '<Leader>mC', map.cmd('MarkClear'),          { desc  = "[C]lear all marks" })
        -- No default mapping for <Plug>MarkConfirmAllClear.
        map.map('n',         '<Leader>mt', '<Plug>MarkToggle',            { desc  = "[t]oggle mark display" })
        map.map('n',         '<Leader>*',  '<plug>MarkSearchCurrentNext', { desc  = "[*] next mark" })
        map.map('n',         '<Leader>#',  '<plug>MarkSearchCurrentPrev', { desc  = "[#] prev mark" })
        map.map('n',         '<Leader>/',  '<plug>MarkSearchAnyNext',     { desc  = "[/] next mark" })
        map.map('n',         '<Leader>?',  '<plug>MarkSearchAnyPrev',     { desc  = "[?] prev mark", unique = true })
        map.map('n',         '*',          '<plug>MarkSearchNext',        { desc  = "[*] next mark (on mark)" })
        map.map('n',         '#',          '<plug>MarkSearchPrev',        { desc  = "[#] prev mark (on mark)" })
